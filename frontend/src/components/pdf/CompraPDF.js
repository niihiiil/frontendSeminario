...
const CompraPDF = ({ compra, configuracion }) => {
      ...
      <Document>
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.section}>
            <Text>Compra #: {compra.id}</Text>
            <Text>Fecha: {formatDate(compra.date)}</Text>
            <Text>Proveedor: {compra.suplier.name}</Text>
            <Text>RUC: {compra.suplier.ruccode}</Text>
            <Text>Empleado: {`${compra.employee.firstName} ${compra.employee.firstLastName}`}</Text>
          </View>
          ...
          <View style={styles.section}>
            ...
            {compra.details.map((detalle, index) => (
              <View key={index} style={styles.row}>
                <Text style={styles.description}>
                  {detalle.product.name}
                  {detalle.isBonus && ' (Bonificación)'}
                </Text>
          ...
          <View style={styles.totals}>
            <View style={styles.totalRow}>
              ...
              <Text>{formatCurrency(compra.subTotal)}</Text>
            </View>
            <View style={styles.totalRow}>
              <Text style={styles.label}>Descuento:</Text>
              <Text>{formatCurrency(compra.discount)}</Text>
            </View>
            <View style={styles.totalRow}>
              <Text style={styles.label}>IVA ({configuracion.ivaPercentageOfSale}%):</Text>
              <Text>{formatCurrency(compra.ivaAmount)}</Text>
            </View>
            <View style={styles.totalRow}>
              <Text style={styles.label}>Total:</Text>
              <Text>{formatCurrency(compra.total)}</Text>
            </View>
          </View>

          {/* Pie de página */}
          <View style={styles.footer}>
            <Text>Documento de Compra</Text>
          </View>
        </Page>
      </Document>
    </PDFViewer>
