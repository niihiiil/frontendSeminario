...
class EntityClass {
  static async login({ email, password }) {
    try {
      const response = await axios.post('https://rsandy-001-site1.atempurl.com/api/authentication/login', {
        email,
        password,
      });
  ...
  static async agregarUsuario(usuarioData) {
    try {
      const response = await axios.post('https://rsandy-001-site1.atempurl.com/api/user', usuarioData);
